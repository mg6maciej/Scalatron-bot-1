// Example Bot #1: The Reference Bot + mg6maciej random modifications
...
/*        if(dontFireAggressiveMissileUntil < bot.time && bot.energy > 100) { // fire attack missile?
            nearestEnemyMaster match {
                ...
                case Some(relPos) =>    // a master is nearby
                    ...
                    bot.set("dontFireAggressiveMissileUntil" -> (bot.time + 1))
            }
        }*/
/*        else
        ...
        if (bot.energy > 100) {
            val dir = XY.fromDirection45(new java.util.Random().nextInt(8))
            bot.spawn(dir, "mood" -> "New")
        }
    }
    ...
    def forSlave(bot: MiniBot) {
        ...
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "New" => reactAsNewBot(bot)
            case s: String => bot.log("unknown mood: " + s)
        }
    }
    
    def reactAsNewBot(bot: MiniBot) {
        //bot.status("-")
        val lastDirection = bot.inputAsIntOrElse("lastDirection", 0)
        val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeViewAsNewSlave(bot.view)
        ...
        bot.view.offsetToNearest('s') match {
            ...
            case None =>
                bot.view.offsetToNearest('b') match {
                    ...
                    case None =>
                        if (bot.energy > 200) {
                            bot.spawn(XY.fromDirection45(new java.util.Random().nextInt(8)), "mood" -> "New")
                        }
                }
        }
    }
    ...
    def analyzeViewAsNewSlave(view: View) = {
        val directionValue = Array.ofDim[Double](8)
        var nearestEnemyMaster: Option[XY] = None
        var nearestEnemySlave: Option[XY] = None
        ...
        for(i <- 0 until cellCount) {
            ...
            if(cellRelPos.isNonZero) {
                ...
                val value: Double = cells(i) match {
                    ...
                    case 'b' => // bad beast: dangerous, but only if very close
                        if(stepDistance < 3) -400 / stepDistance else -50 / stepDistance
